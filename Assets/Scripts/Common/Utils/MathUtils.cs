...
public static class MathUtils {
    ...
    static public float RoundTo1DP(float _value) {
        return Mathf.Round(_value * 10f) / 10f;
    }
    ...
    /// Converts val to a bool using Sin01. Useful for flashing timers between two colors.
    static public bool Sin01Bool(float val) { return Sin01(val) < 0.5f; }
    ...
    /** Returns TRUE if these rotations are about the same. E.g. (180,0,0) and (0,180,180). */
    public static bool AreEulerAnglesSame(Vector3 vA, Vector3 vB) {
        Quaternion qA = Quaternion.Euler (vA.x,vA.y,vA.z);
        Quaternion qB = Quaternion.Euler (vB.x,vB.y,vB.z);
        return IsSameAngle(qA, qB);
    }
    public static bool IsSameAngle(Quaternion qA,Quaternion qB) {
        float angle = Quaternion.Angle (qA,qB);
        return Mathf.Abs(angle) < 0.1f;
    }
    ...
    public static Vector2Int GetDir(int side) {
        switch (side) {
            ...
            case Sides.D: return Vector2Int.D;
            case Sides.U: return Vector2Int.U;
    ...
    public static int GetSide(Vector2Int dir) {
        ...
        if (dir == Vector2Int.U) { return Sides.U; }
        if (dir == Vector2Int.D) { return Sides.D; }
    ...
    public static Line GetRectSideLine(Rect r, int side) {
        switch (side) {
            ...
            case Sides.D: return new Line(r.xMin,r.yMin, r.xMax,r.yMin);
            case Sides.U: return new Line(r.xMin,r.yMax, r.xMax,r.yMax);
    ...
    public static int GetSideRectIsOn (Rect rectA, Rect rectB) {
        ...
        if (largestValue == diffB) { return Sides.D; }
        if (largestValue == diffT) { return Sides.U; }
        return -1; // impossibru!!
    }
